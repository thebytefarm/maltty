...
describe(parseSgrMouse, () => {
  ...
  it('should parse an Ink input value without its escape prefix', () => {
    expect(parseSgrMouse('[<0;1;1M')[0]).toMatchObject({ type: 'down', x: 0, y: 0 })
  })
  ...
  it('should parse release events', () => {
    expect(parseSgrMouse('\u001B[<2;4;3m')[0]).toMatchObject({ button: 'right', type: 'up' })
  })

  it('should parse modified drag events', () => {
    expect(parseSgrMouse('\u001B[<61;4;3M')[0]).toMatchObject({
      button: 'middle',
  ...
  it('should parse buttonless movement', () => {
    expect(parseSgrMouse('\u001B[<35;4;3M')[0]).toMatchObject({
      button: 'none',
      type: 'move',
    })
  })
  ...
  it.each([
    ...
    expect(parseSgrMouse(`\u001B[<${code};4;3M`)[0]).toMatchObject({
      button: 'none',
  ...
  it('should preserve coordinates wider than the X10 byte range', () => {
    expect(parseSgrMouse('\u001B[<0;1200;800M')[0]).toMatchObject({ x: 1199, y: 799 })
  })

  it.each(['', '\u001B[<0;1', 'hello', '\u001B[<left;1;1M'])(
    'should reject malformed sequence %j',
    (input) => {
      expect(parseSgrMouse(input)[1]).toMatchObject({ type: 'invalid_sequence' })
    }
  )

  it('should reject zero coordinates', () => {
    expect(parseSgrMouse('\u001B[<0;0;1M')[1]).toMatchObject({ type: 'invalid_coordinates' })
  })

  it('should reject unsafe button codes', () => {
    expect(parseSgrMouse('\u001B[<999999999999999999999;1;1M')[1]).toMatchObject({
      type: 'invalid_code',
    })
  })

  it.each([128, 129, 130, 131])('should reject unsupported extended button code %i', (code) => {
    expect(parseSgrMouse(`\u001B[<${code};1;1M`)[1]).toMatchObject({ type: 'invalid_code' })
  })

  it('should reject a buttonless press', () => {
    expect(parseSgrMouse('\u001B[<3;1;1M')[1]).toMatchObject({ type: 'invalid_code' })
  })
})
