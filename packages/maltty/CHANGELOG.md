...
- c84b4dc: Expose the `maltty/store`, `maltty/project`, and `maltty/format` subpath exports. These modules were already built and documented, but the `exports` map omitted them, so the imports failed to resolve.
